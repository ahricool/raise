...
test_syntax() {
    ...
    python3 -m py_compile api/app.py src/config.py src/notification.py \
...
test_flake8() {
    ...
    if command -v flake8 &> /dev/null; then
        flake8 api/app.py src/config.py src/notification.py --select=F821,E999 --max-line-length=120
        success "Flake8 检查通过"
    else
